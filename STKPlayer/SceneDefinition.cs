...
namespace STKPlayer
{
    ...
    public class SceneDefinition
    {
        ...
        {
            _startMS = Utilities.Frames15fpsToMS(frameStart - 2);
            _endMS = Utilities.Frames15fpsToMS(frameEnd - 2);
            _offsetTimeMS = sOffset * 100;
            SceneType = type;
            Name = name;
        }
        public SceneDefinition(SceneType type, string name, int frameStart, int frameEnd, long sOffset, int successFrame, int retryFrame)
        {
            ...
            _endMS = Utilities.Frames15fpsToMS(frameEnd - 2);
            _offsetTimeMS = sOffset*100;
            _successMS = Utilities.Frames15fpsToMS(successFrame - 2);
            _retryMS = Utilities.Frames15fpsToMS(retryFrame - 2);
