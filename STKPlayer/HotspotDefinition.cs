    ...
    public class HotspotDefinition
    {
        ...
        public bool HitTest( int X, int Y, long Milliseconds, SceneDefinition scene)
        {
            ...
            switch (HType)
            {
                ...
                case HotspotType.Multi:
                    if (Area.Count == 1)
                    {
                        if (X >= (Area[0].TopLeft.X * _Scale) && X <= (Area[0].BottomRight.X * _Scale) && Y >= (Area[0].TopLeft.Y * _Scale) && Y <= (Area[0].BottomRight.Y * _Scale) && InFrame)
                            return true;
                        return false;
                    }
                    if (Area.Count == 2)
                    {
                        ...
                        if (X >= (interpolatetopleftX * _Scale) && X <= (interpolatebottomrightX * _Scale) && Y >= (interpolatetopleftY * _Scale) && Y <= (interpolatebottomrightY * _Scale) && InFrame)
                            return true;
                        return false;
                    }
                    break;
        ...
        public void Draw(Grid parentElement, long Milliseconds, SceneDefinition scene)
        {
            ...
            switch (HType)
            {
                ...
                case HotspotType.Multi:
                    if ( Area.Count ==1 )
                    left = Area[0].TopLeft.X * _Scale;
                    top = Area[0].TopLeft.Y * _Scale;
                    right = Area[0].BottomRight.X * _Scale;
                    bot = Area[0].BottomRight.Y * _Scale;
